...
//   const { is } = await dynamicImport('@wromojs/compiler/utils');
//   try {
...
export function isOrCanBeConvertedToShorthand(node: AttributeNode, opts: ParserOptions): boolean {
	if (!opts.wromoAllowShorthand) return false;
...
/**
 *  True if node is of type `{a}` and wromoAllowShorthand is false
 */
...
export function getMarkdownName(script: string): Set<string> {
	...
	let defaultMatch;
	while (
		(defaultMatch = /import\s+([^\s]+)\s+from\s+['|"|`]wromo\/components\/Markdown\.wromo/g.exec(
			script
		))
	) {
	...
	let namedMatch;
	while ((namedMatch = /import\s+\{\s*([^}]+)\}\s+from\s+['|"|`]wromo\/components/g.exec(script))) {
